...
interface WhatiffInsightsProps {
  calculatorType: 'retirement' | 'sip' | 'emi' | 'goal' | 'fd' | 'staggered-fd' | 'loan-affordability' | 'home-purchase' | 'buy-vs-rent' | 'prepay-vs-invest' | 'child-future-planner';
  results: any;
  ...
  const renderBold = (text: string) => {
    return text.split(/(\*\*.*?\*\*)/g).map((part, i) => {
      if (part.startsWith('**') && part.endsWith('**')) {
        return <strong key={i} className="text-emerald-500 font-bold">{part.slice(2, -2)}</strong>;
      }
      return part;
    });
  };
